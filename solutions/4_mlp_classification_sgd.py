...
def main(args: argparse.Namespace) -> tuple[tuple[np.ndarray, ...], list[float]]:
    ...
    def forward(inputs):
        ...
        hidden_activation_ReLU = np.maximum(hidden_layer, 0)

        output_layer = np.dot(hidden_activation_ReLU, weights[1]) + biases[1]

        shift_values = np.exp(output_layer - np.max(output_layer, axis=1, keepdims=True))
        output_activation_softmax = shift_values / np.sum(shift_values, axis=1, keepdims=True) 

        return hidden_activation_ReLU, output_activation_softmax
    ...
    for epoch in range(args.epochs):
        ...
        for i in range(0, number_train_examples, args.batch_size):
            ...
            output_gradient = output_layer
            output_gradient[np.arange(args.batch_size), batch_target] -= 1
            output_gradient /= args.batch_size

            hidden_gradient = np.dot(output_gradient, weights[1].T)
            hidden_gradient[hidden_layer <= 0] = 0

            grad_weights_output = np.dot(hidden_layer.T, output_gradient)
            grad_biases_output = np.sum(output_gradient, axis=0)

            grad_weights_hidden = np.dot(batch_data.T, hidden_gradient)
            grad_biases_hidden = np.sum(hidden_gradient, axis=0)

            weights[1] -= args.learning_rate * grad_weights_output
            biases[1] -= args.learning_rate * grad_biases_output

            weights[0] -= args.learning_rate * grad_weights_hidden
            biases[0] -= args.learning_rate * grad_biases_hidden

        # TODO: After the SGD epoch, measure the accuracy for both the
        # train test and the test set.
        # Calculate accuracy for training and test sets.
        mock, train_predictions = forward(train_data)
        train_accuracy = np.mean(np.argmax(train_predictions, axis=1) == train_target)

        mock, test_predictions = forward(test_data)
