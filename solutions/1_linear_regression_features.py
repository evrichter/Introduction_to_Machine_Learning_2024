...
def main(args: argparse.Namespace) -> list[float]:
    ...
    for order in range(1, args.range + 1):
        ...
        X = np.column_stack([xs**i for i in range(1, order + 1)])

        # DONE: Split the data into a train set and a test set.
        # Use `sklearn.model_selection.train_test_split` method call, passing
        # arguments `test_size=args.test_size, random_state=args.seed`.
        train_data, test_data, train_target, test_target = sklearn.model_selection.train_test_split(X,ys, test_size=args.test_size, random_state=args.seed)

        # DONE: Fit a linear regression model using `sklearn.linear_model.LinearRegression`;
        ...
        rmse = sklearn.metrics.root_mean_squared_error(test_target, y_pred)
